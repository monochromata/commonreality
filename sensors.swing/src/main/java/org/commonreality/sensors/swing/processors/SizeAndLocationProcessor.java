...
public class SizeAndLocationProcessor extends AbstractComponentProcessor {
	 ...
	 * Logger definition
	 */
	static private final transient Log LOGGER = LogFactory.getLog(SizeAndLocationProcessor.class);
	...
	public void configure(Map<String, String> properties) {
		try {
			_viewerPhysicalDistance = Double.parseDouble(properties.get(VIEWER_DISTANCE_PARAM));
		} catch (Exception e) {
			LOGGER.warn("Could not extract viewer distance from properties, assuming 0.75", e);
			_viewerPhysicalDistance = 0.75;
		}

		try {
			_screenPhysicalHeight = Double.parseDouble(properties.get(SCREEN_HEIGHT_PARAM));
			_screenPhysicalWidth = Double.parseDouble(properties.get(SCREEN_WIDTH_PARAM));
		} catch (Exception e) {
			LOGGER.warn("Could not extract physical screen size, assuming 0.3 x 0.2", e);
			_screenPhysicalHeight = 0.25;
			_screenPhysicalWidth = 0.3;
		}

		_viewerPhysicalDistanceSquared = _viewerPhysicalDistance * _viewerPhysicalDistance;
	...
	/**
	 * sets the retinal location, distance and size properties of a visual
	 * percept
	 */
	public void process(AWTObjectKey object, IMutableObject simulationObject) {
		...
		simulationObject.setProperty(IVisualPropertyHandler.RETINAL_DISTANCE, dimensions[2]);
	...
	public double[] toScreenLocation(double[] location) {
		...
		if (LOGGER.isDebugEnabled())
			LOGGER.debug(String.format("retino (%.2f, %.2f) to screen (%.2f, %.2f)", location[0], location[1], rtn[0],
					rtn[1]));
	...
	public double[] toRetinotopic(Component component) {
		...
		if (LOGGER.isDebugEnabled())
			LOGGER.debug(String.format("%s is at (%d, %d) screen", component.hashCode(), origin.x, origin.y));

		/*
		 * now let's convert that coordinate system to 0,0 center, +x right +y
		 * up.
		 */
		...
		if (LOGGER.isDebugEnabled())
			LOGGER.debug(String.format("%s is at (%.2f, %.2f, %.2f) meters", component.hashCode(), ox, oy, oz));
		...
		double[] rtn = new double[] { ox + width / 2, oy - height / 2, (oz + lrz / 2), width, height };

		if (LOGGER.isDebugEnabled())
			LOGGER.debug(String.format("%s is at (%.2f, %.2f, %.2f) x (%.2f, %.2f) retino", component.hashCode(),
